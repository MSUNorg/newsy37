	...
	<div class="cf">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
				...
				<button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('del?model='.$model['id']);?>">删 除</button>
                 ...
                 <a class="btn" id="dtn" href="javascript:;">批量充值</a>
            <a class="btn" id="dtnn" href="javascript:;">批量删除</a>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			<div class="sleft">
				<input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'user_nickname'); ?>" class="search-input" value="<?php echo I('user_nickname');?>" placeholder="请输入用户昵称">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Provide/lists','model='.$model['name'],false);?>"><i class="btn-search"></i></a>
			</div>
            ...
            <table>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            <td><input class="ids" type="checkbox" value="<?php echo ($data['id']); ?>" name="ids" status="<?php echo ($data['status']); ?>"></td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Provide/lists');?>');
$(function(){
     $('.ids').ready(function(){
  ...
  var spCodesTemp = "";
      ...
      if(spCodesTemp==""){
        ...
        $(this).attr('href','/newswht/admin.php?s=/Provide/batch/ids/'+spCodesTemp);
      }
  ...
  var spCodesTemp = "";
      ...
      if(spCodesTemp==""){
        ...
        $(this).attr('href','/newswht/admin.php?s=/Provide/delprovide/ids/'+spCodesTemp);
      }
