...
<body>
	...
	<div class="cf">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
				 <button class="btn ajax-post lock" target-form="ids" url="<?php echo U("Promote/set_status",array('model'=>'User',"status"=>1,"msg_type"=>8,"field"=>"lock_status"));?>">解 锁</button>
            <button class="btn ajax-post unlock" target-form="ids" url="<?php echo U("Promote/set_status",array('model'=>'User',"status"=>0,"msg_type"=>8,"field"=>"lock_status"));?>">锁 定</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
			...
			<div class="sleft">
				<input type="text" name="account" class="search-input" value="<?php echo I('account');?>" placeholder="请输入会员账号">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/ch_reg_list','model='.$model['name'],false);?>"><i class="btn-search"></i></a>
			</div>
		</div>
	</div>
            ...
            <table>
                ...
                <thead>
                    <tr>
                        ...
                        <th style="text-align:center">锁定状态</th>
                ...
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo get_status_text(1,$data['lock_status']);?></td>
                            ...
                            <td style="border-right:1px solid #DDDDDD;text-align:center"><?php echo ($data["promote_account"]); ?></td>
                            <td style="border-right:1px solid #DDDDDD;text-align:center">
                                <?php echo get_promote_type($data['promote_id']);?>
                            </td>
                            <td style="border-right:1px solid #DDDDDD;text-align:center">
                                <?php echo get_belong_admin($data['promote_id']);?>
                            </td>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Promote/ch_reg_list');?>');
$(function(){
	//搜索功能
