...
<body>
	...
	<div class="cf">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
                ...
                <button class="btn ajax-post org" target-form="ids" url="<?php echo U("Apply/set_status",array("status"=>1,"msg_type"=>5,"field"=>"status"));?>">审 核</button>
                <button class="btn ajax-post bohui" target-form="ids" url="<?php echo U("Apply/set_status",array("status"=>2,"msg_type"=>6,"field"=>"status"));?>">驳 回</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
		<div class="search-form fr cf">
            ...
            <div id="apply" class="sleft">
                <div  class="drop-down">
                    <span id="sch-sort-txt" class="sort-txt" data="<?php echo I('status');?>">
                        <?php if(I('status') == ''): ?>所有
                        <?php elseif(I('status') == 1): ?>审核
                        <?php elseif(I('status') == 2): ?>驳回
                        <?php else: ?>未审核<?php endif; ?>
                    </span>
                    <i class="arrow arrow-down"></i>
                    <ul id="sub-sch-menu" class="nav-list hidden">
                        <li><a href="javascript:;" value="">所有</a></li>
                        <li><a href="javascript:;" value="1">审核</a></li>
                        <li><a href="javascript:;" value="2">驳回</a></li>
                        <li><a href="javascript:;" value="0">未审核</a></li>
                    </ul>
                </div>
                <input type="hidden" id="status" name="status"  value="<?php echo I('status');?>" >
            </div>
			<div class="sleft">
				<input type="text" name="game_name" class="search-input" value="<?php echo I('game_name');?>" placeholder="请输入游戏名称">
				<a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Apply/lists','model='.$model['name'],false);?>"><i class="btn-search"></i></a>
			</div>
