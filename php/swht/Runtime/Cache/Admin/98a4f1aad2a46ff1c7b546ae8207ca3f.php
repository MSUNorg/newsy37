...
<!-- 标题栏 -->
    ...
    <div class="cf">
        ...
        <?php if(empty($model["extend"])): ?><div class="tools">
                <a class="btn payway" href="<?php echo U('pay_way?type=4');?>">全 部</a>
                <a class="btn payway" href="<?php echo U('pay_way?type=0');?>">本 月</a>
                <a class="btn payway" href="<?php echo U('pay_way?type=1');?>">上 月</a>
                <a class="btn payway" href="<?php echo U('pay_way?type=2');?>">本 周</a>
                <a class="btn payway" href="<?php echo U('pay_way?type=3');?>">上 周</a>
            </div><?php endif; ?>
        </div>
        <!-- 高级搜索 -->
        <!-- <div class="search-form fr cf">
            <div class="btn-group-click adv-sch-pannel fl">
                ...
                <div class="dropdown cf">
                    <div class="row">
                        <label>时间：</label>
                        <input type="text" id="time-start" name="StartTime" class="text input-2x" value="" placeholder="起始时间" /> -                     
                        <div class="input-append date" id="datetimepicker"  style="display:inline-block">
                            <input type="text" id="time-end" name="LastTime" class="text input-2x" value="" placeholder="结束时间" />
    ...
    <div class="data-table">
        <div class="data-table table-striped">
            <table>
                <!-- 表头 -->
                <thead>
                    <tr><th colspan="4" class="tabtit">系统支付数据|银行账户数据</th></tr>
                    <tr>
                      <th>日期</th><th>支付宝</th><th>微信</th><th>总计</th>
                    </tr>
                </thead>
                <!-- 列表 -->
                <tbody>
                    <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                        <td><?php echo ($data["PayTime"]); ?></td>
                        <td><?php echo sprintf("%.2f",$data['alipay']);?>元</td>
                        <td><?php echo sprintf("%.2f",$data['wxpay']);?>元</td>
                        <td><?php echo sprintf("%.2f",$data['total']);?>元</td>
                    </tr><?php endforeach; endif; else: echo "" ;endif; ?>
                    <tr class="taball">
                        <td>总计</td>  
                        <td><?php echo sprintf("%.2f",$alipay_total);?>元</td>
                        <td><?php echo sprintf("%.2f",$weixin_total);?>元</td>
                        <td><?php echo sprintf("%.2f",$total);?>元</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<script type="text/javascript">
//导航高亮
highlight_subnav('<?php echo U('Stat/pay_way');?>');
</script>
