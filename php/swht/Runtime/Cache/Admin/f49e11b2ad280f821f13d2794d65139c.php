...
<body>
	...
	<div class="cf">
		...
		<?php if(empty($model["extend"])): ?><div class="tools">
				<!-- <a class="btn" href="<?php echo U('add_media_adv?model='.$model['id']);?>">新 增</a> -->
                <button class="btn ajax-post confirm red" target-form="ids" url="<?php echo U('del_adv?model='.$model['id']);?>">删 除</button>
			</div><?php endif; ?>
		</div>
		<!-- 高级搜索 -->
        ...
        <div class="data-table table-striped">
            <table>
                ...
                <?php if(is_array($list_data)): $i = 0; $__LIST__ = $list_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo get_adv_pos_title($data['pos_id']);?></td>
                        <td><?php echo set_show_time($data['start_time']);?></td>
                        <td><?php echo set_show_time($data['end_time']);?></td>
                        <td>
                            
                        </td>
                        <td>
                            <?php switch($data['target']): case "_blank": ?>新页面<?php break;?>
                                <?php case "_self": ?>本页面<?php break;?>
                                <?php default: ?>未知<?php endswitch;?></td>
                        <td>
                            <a href="<?php echo U('edit_adv',array('id'=>$data['id'],'model'=>$model['id']));?>">编辑</a>
                            <a href="<?php echo U('del_adv?ids='.$data['id']);?>" class="ajax-get confirm" target-form="ids">删除</a>
                        </td>
...
$(function(){
    //导航高亮
    highlight_subnav('<?php echo U('Adv/adv_lists');?>');
