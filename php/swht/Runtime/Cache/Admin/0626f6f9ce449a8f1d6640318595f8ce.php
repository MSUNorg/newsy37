...
<body>
    ...
    <div class="cf">
        ...
        <?php if(empty($model["extend"])): ?><div class="tools">
                ...
                <button class="btn ajax-post confirm" target-form="ids" url="<?php echo U('agent_del?model='.$model['id']);?>">删 除</button>
            </div><?php endif; ?>
        </div> -->
        <!-- 高级搜索 -->
        <div class="search-form fr cf">
            <div class="sleft">
                <input type="text" name="<?php echo ((isset($model['search_key']) && ($model['search_key'] !== ""))?($model['search_key']):'user_account'); ?>" class="search-input" value="<?php echo I('user_account');?>" placeholder="请输入充值账号">
                <a class="sch-btn" href="javascript:;" id="search" url="<?php echo U('Promote/agent_list','model='.$model['name'],false);?>"><i class="btn-search"></i></a>
            </div>
