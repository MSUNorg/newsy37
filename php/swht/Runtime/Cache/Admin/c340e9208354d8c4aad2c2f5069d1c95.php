...
<body>
    ...
    <div>
        <button class="btn blue" id="action_add" url="<?php echo U('addaction');?>">新 增</button>
        <button class="btn ajax-post gre" target-form="ids" url="<?php echo u('setstatus?Model=Action&status=1');?>" >启 用</button>
        <button class="btn ajax-post red" target-form="ids" url="<?php echo u('setstatus?Model=Action&status=0');?>">禁 用</button>
        <button class="btn ajax-post confirm red" target-form="ids" url="<?php echo U('setStatus?Model=Action&status=-1');?>">删 除</button>
    </div>
	<!-- 数据列表 -->
