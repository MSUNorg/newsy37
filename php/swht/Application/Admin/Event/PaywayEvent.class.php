...
class PaywayEvent extends Controller {
    ...
    */
    ...
    public function group_data($str_where){
        $data = M()->query("select pay_time,sum(case pay_way when '0' then pay_amount else 0 end) as alipay,sum(case pay_way when '1' then pay_amount else 0 end) as wxpay,sum(case pay_way when '2' then pay_amount else 0 end) as pintai,sum(pay_amount) as total from tab_spend ".$str_where." group by DATE_FORMAT(pay_time,'%Y%m%d') order by pay_time DESC");
