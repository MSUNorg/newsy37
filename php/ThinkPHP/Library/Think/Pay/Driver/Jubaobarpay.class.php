...
class Jubaobarpay extends \Think\Pay\Pay {

    protected $gateway = 'http://www.jubaopay.com/apipay.htm';
    ...
    public function buildRequestForm(\Think\Pay\PayVo $vo) {
        //商户利用支付订单（payid）和商户号（mobile）进行对账查询 $_SERVER["ROOT_DOCUMENT"]
        $jubaopay = new jubaopay($_SERVER['DOCUMENT_ROOT'].'/Application/Sdk/SecretKey/jubaopay/jubaopay.ini');
        $jubaopay->setEncrypt("payid", $vo->getOrderNo());
        $jubaopay->setEncrypt("partnerid", "16070723351477716966");
        $jubaopay->setEncrypt("amount", $vo->getFee());
        $jubaopay->setEncrypt("payerName", $vo->getTitle());
        $jubaopay->setEncrypt("remark", $vo->getBody());
        $jubaopay->setEncrypt("returnURL", "");
        $jubaopay->setEncrypt("callBackURL", $this->config['notify_url']);
