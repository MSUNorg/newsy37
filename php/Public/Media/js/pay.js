...
{
    var that = $('#create_user_pay_order');
...
/*    alert(pay_type);*/
    if(pay_type == 'alipay'){
        $('input[name="pay_money"]').val(g_form_data.money);
        $('#create_user_pay_order').submit();
    }
    if(pay_type == 'jubaopay' || pay_type =='swiftpass'){
    	$('input[name="pay_money"]').val(g_form_data.money);
    	$('#create_user_pay_order').prop('action',$("#jubaopay").val());
    	$('#create_user_pay_order').submit();
    }

/*    var url = "{:U('Recharge/check')}";
    $.post(
        'media.php/Recharge/check',
      {
        'data':g_form_data.username,
        'pay_money':g_form_data.money
      },
      function(msg){
        alert(msg);
