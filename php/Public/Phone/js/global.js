var SURL = '/phone.php?s=/',
	UURL = '/phone.php?s=/Subscriber/';
// xigu  lwx
(function() {
    ...
    (function() {
        ...
        XGUC.is_login = function() {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    case 1:
						...
						$('#topShowName').text('您好，' + data.username);
						$('.js-vip-icon.icon-m-vip-smaller').removeClass('icon-m-vip-smaller-0').
						addClass('icon-m-vip-smaller-'+data.viplevel);
						$('.pop-login-register ul li').before('<li><a href="'+UURL+'index">用户中心</a></li>');
						return;
                        break;
                    default:
			...
			$('.login-register,.pop-login-register').on('mouseover',function() {
                $(this).hasClass('logon') && ($(this).addClass('active'), $('.pop-login-register').addClass('active'))
            });
			
			$('.login-register,.pop-login-register').on('mouseleave',function() {
		...
		XGUC.phone_reg = function(form_id) {
			...
			var s = function(phone) {
				...
				$('#getSafeCodePop').on('click',function() {
					// 发送安全码
			...
			$('#registerPhonePop').blur(function() {
				...
				if ((/^[1][358][0-9]{9}/.test(phone))) s(phone); else f('registerPhonePop','手机号码格式不正确',true);
			});
			...
			$('#userNameByNamePop').blur(function() {
				...
				$.post(UURL+'checkUser',{username:name},function(data){
					if (parseInt(data.status) != 1) {
						f('userNameByNamePop',data.info);return;
					}
					return;
				});
			});
