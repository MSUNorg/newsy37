...
class DocumentModel extends Model{
	...
	public function listspage($category, $order = '`id` DESC', $status = 1, $field = true,$page=1,$limit=10){
		$map = $this->listMap($category, $status);
		$page=I('p')?I('p'):1;
		$count      = $this->where(array('status'=>1,'category_id'=>$category))->count();// 查询满足要求的总记录数
		$Page       = new \Think\Page($count,$limit);// 实例化分页类 传入总记录数和每页显示的记录数
		$data['show']       = $Page->show();// 分页显示输出
		$data['list']=$this->field($field)->where($map)->order($order)->page($page, $limit)->select();
		return $data;
	}
	...
	*获取文章实体
	*/
	public function decument_entity_top($category, $order = '`id` DESC', $status = 1, $field = true,$page=1,$limit=1){
		$map = $this->listMap($category, $status);
		return $this->field('sys_document.id,sys_document.cover_id,sys_document.title,sys_document_article.content')
					->join("LEFT JOIN sys_document_article ON sys_document.id = sys_document_article.id")
	...
	public function detail($id){
		/* 获取基础数据 */
/*		$info = $this->table('sys_document_article')->join("LEFT JOIN sys_document ON tab_giftbag.game_id = tab_game.id")->field(true)->find($id);
		if(!(is_array($info) || 1 !== $info['status'])){
			$this->error = '文档被禁用或已删除！';
			return false;
		}*/
		$data=$this->field('sys_document.id,sys_document.title,sys_document.update_time,sys_document_article.content')
					->join("LEFT JOIN sys_document_article ON sys_document.id = sys_document_article.id")
					->where(array('sys_document.id'=>$id))->find();
	...
	//首页新闻栏,根据产品需求,每种文章取出三个展示
	public function newslist(){

		$sql="SELECT a.id,a.category_id,a.title,c.title cate_title
	            FROM sys_document AS a
	       LEFT JOIN sys_document AS b ON a.category_id = b.category_id AND a.id < b.id
	       LEFT JOIN sys_category AS c ON a.category_id = c.id
	           WHERE c.name IN ('media_new','media_activity','media_gg') AND a.status = 1
	        GROUP BY a.id,a.category_id
	          HAVING COUNT(b.id) < 3
	        ORDER BY a.category_id DESC,a.level DESC,a.create_time DESC";
