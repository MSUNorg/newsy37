...
class SubscriberController extends BaseController {
	...
	*/
	...
	public function telsvcode($phone=null,$verify='',$delay=10,$flag=false) {
        ...
        /* $xigu = new Xigu(C('MP_USER_ID'));
        $param = $rand.",".$delay;
        $result = json_decode($xigu->sendSM(C('MP_APP_ID'),$phone,C('MP_TEMPLATE_ID'),$param),true); */
...
/** * 领取礼包 */	
	public function getGameGift($giftid,$giftname,$gameid,$gamename) {		
		...
		if($users) {   
			...
			if($info) {
				$data=$info['novice'];
				$this->ajaxReturn(array('status'=>'2','info'=>'不要太贪心啊，给别人一个领取的机会吧','url'=>'','data'=>$data));
			} else {
				$giftbag = D('Giftbag');
				$n = $giftbag->field('novice')->where("id=$giftid")->find();
				$n = $n['novice'];
				if (empty($n)) {
					$this->ajaxReturn(array('status'=>'0','info'=>'你来晚了一步，礼包已被领取完了','url'=>'','data'=>''));	
				} else {					
					$novice = explode(",",$n);
					$guid = $novice[0];										
					$data['game_id']=$gameid;
					...
					$this->ajaxReturn(array('status'=>'1','info'=>'恭喜你，领取成功！','url'=>'','data'=>$guid));
				}
			}    
		} else {
	...
	public function register() {
		...
		if (IS_POST) {
			if(!check_verify($_POST['verify'])){
				return $this->ajaxReturn(array('status'=>0,'info'=>'验证码错误'));
			}
