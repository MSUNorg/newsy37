...
class UserModel extends Model{
    ...
    public function login_($account,$password,$type,$game_id,$game_name){
        $map['account'] = $account;
        /* 获取用户数据 */
        $user = $this->where($map)->find();
        if(is_array($user) && $user['lock_status']){
            /* 验证用户密码 */
            if(think_ucenter_md5($password, UC_AUTH_KEY) === $user['password']){
                $this->updateLogin_($user['id'],$user['fgame_id'],$game_id,$game_name); //更新用户登录信息
                $this->user_login_record($user,$type,$game_id,$game_name);
                return $user['id']; //登录成功，返回用户ID
            } else {
                return -2; //密码错误
    ...
    *user表加game_id
    */
    public function register_($account,$password,$register_way,$promote_id=0,$promote_account="",$phone="",$game_id="",$game_name=""){
        $data = array(
            'account'    => $account,
            'password'   => $password,
            'nickname'   => $account,
    ...
    */
    ...
    //判断game_id是否有值
    protected function updateLogin_($uid,$user_fgame_id,$game_id,$game_name){
        $model = M('User','tab_');
        $data["id"] = $uid;
        $data["login_time"] = NOW_TIME;
        $data["login_ip"] = get_client_ip();
