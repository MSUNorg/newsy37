...
class ApplyController extends ThinkController {
    ...
    {
        try{
            $ids || $this->error("打包数据不存在",U('Apply/lists'),1);
            static $a=0;//无数据或没审核
            static $b=0;//原包不存在
            static $c=0;//成功
            static $d=0;//操作失败
            ...
            if ($res==TRUE) {
                ...
                $jieguo = $this->updateinfo($value,$to,$promote);
                if($jieguo){
                    $c++;
                }
                else{
            ...
            $f=$a+$b+$d+$e;
            $this->success('成功'.$c.',失败'.$f,U('lists'));
    ...
    public function package($ids=null)
    {
        try{
            ...
            //验证原包是否存在
            if(empty($source_file) || !file_exists($source_file['file_url'])){$this->error("游戏原包不存在",U('Apply/lists')); exit();}
